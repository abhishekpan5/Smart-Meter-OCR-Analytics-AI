...
class GPT4VisionMeterReader:
    ...
    def validate_against_database(self, extraction_results: List[Dict]) -> List[Dict]:
        """
        Enhanced validation including date/time comparison against database records and partial/weighted matching
        """
        try:
            ...
            for result in extraction_results:
                ...
                try:
                    ...
                    continue
                # 1. Try exact serial+reading match
                if extracted_serial:
                    cursor.execute("""
                        SELECT meter_serial_number, meter_reading, reading_unit, reading_date
                        FROM meter_readings
                    ...
                    if exact_serial_matches:
                        match_status = "exact"
                        matched_fields = ["meter_reading", "meter_serial"]
                        notes.append("Exact match on reading and serial number.")
                        closest_db_records = exact_serial_matches
                # 2. Try partial serial match (prefix or suffix, at least 5 chars)
                if match_status == "none" and extracted_serial and len(extracted_serial) >= 5:
                    cursor.execute("""
                        SELECT meter_serial_number, meter_reading, reading_unit, reading_date
                        FROM meter_readings
                        WHERE meter_reading = ? AND (meter_serial_number LIKE ? OR meter_serial_number LIKE ?)
                        ORDER BY reading_date DESC LIMIT 5
                    """, (str(reading), f"{extracted_serial}%", f"%{extracted_serial}"))
                    partial_serial_matches = cursor.fetchall()
                    if partial_serial_matches:
                        match_status = "partial_serial"
                        matched_fields = ["meter_reading", "meter_serial_partial"]
                        unmatched_fields = ["meter_serial"]
                        notes.append(f"Partial serial match: extracted '{extracted_serial}' matches prefix/suffix in DB.")
                        closest_db_records = partial_serial_matches
                # 3. Try reading+date proximity match (within 1 day)
                if match_status == "none" and extracted_date:
                    cursor.execute("""
                        SELECT meter_serial_number, meter_reading, reading_unit, reading_date
                        FROM meter_readings
                        WHERE meter_reading = ?
                    ...
                    """, (str(reading),))
                    reading_matches = cursor.fetchall()
                    for db_row in reading_matches:
                        db_date = db_row[3]
                        date_comp = self._compare_dates(extracted_date, db_date)
                        if date_comp.get("match_level") in ["exact", "close"]:
                            match_status = "reading_datetime"
                            matched_fields = ["meter_reading", "date_time"]
                            unmatched_fields = ["meter_serial"]
                            notes.append(f"Reading matches and date/time is {date_comp['match_level']} (difference: {date_comp.get('days_difference','?')} days). Serial does not match.")
                            closest_db_records = [db_row]
                            break
                # 4. Fallback: reading only match
                if match_status == "none":
                    cursor.execute("""
                        ...
                        ORDER BY reading_date DESC LIMIT 5
                    """, (str(reading),))
                    reading_only_matches = cursor.fetchall()
                    if reading_only_matches:
                        match_status = "reading_only"
                        matched_fields = ["meter_reading"]
                        unmatched_fields = ["meter_serial", "date_time"]
                        notes.append("Reading matches, but serial and date/time do not match or are missing.")
                        closest_db_records = reading_only_matches
                # 5. No match at all
                if match_status == "none":
                    unmatched_fields = ["meter_reading", "meter_serial", "date_time"]
                    notes.append("No matching record found in database.")
                # Format closest_db_records for output
                formatted_db_records = [
                    {
                        ...
                        "db_unit": row[2],
                        "db_date": row[3],
                        "date_comparison": self._compare_dates(extracted_date, row[3]) if extracted_date else None
                    }
                    for row in closest_db_records
                ]
                validation_result = {
                    ...
                    "closest_db_records": formatted_db_records,
                    "notes": notes,
    ...
    def generate_enhanced_accuracy_report(self, validation_results: List[Dict]) -> Dict:
        ...
        successful_matches = sum(1 for r in validation_results if r.get("match_status") == "exact")
        ...
        for result in validation_results:
            ...
            # Check for matches using the new structure
            if result.get("closest_db_records"):
                exact_matches += 1 if result.get("match_status") == "exact" else 0
                close_matches += 1 if result.get("match_status") in ["partial_serial", "reading_datetime"] else 0
